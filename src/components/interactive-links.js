...
function initInteractiveLinks(containerId) {
  ...
  const customLinks = window.CONFIG?.custom_links || [];

  const hasAnySocial = Object.values(social).some(url => url && url.trim());
  const hasCustomLinks = customLinks.length > 0;

  if (!hasAnySocial && !hasCustomLinks) return;

  const wrapper = document.createElement('div');
  wrapper.className = 'interactive-links-wrapper';
    ...
    Object.entries(social).forEach(([platform, url]) => {
      ...
      html += `
        <a
          ...
          aria-label="${platform}"
          ${isGradient ? `style="background: ${color};"` : `style="background-color: ${color};"`}
        >
