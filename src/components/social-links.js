 ...
 * Renders social media icons + custom link buttons
 */
  ...
  return `
    <a 
      ...
      style="
        ...
        height: 44px;
        border-radius: 50%;
        background: ${isGradient ? `url(#${gradientId})` : config.color};
...
function createCustomLinkButton(link) {
...
function initInteractiveLinks(containerId) {
  ...
  const activePlatforms = Object.entries(social)
    .filter(([platform, url]) => url && url.trim() !== '')
    .map(([platform, url]) => ({ platform, url }));

  // Build HTML
  let html = '<div class="interactive-links-wrapper" style="width: 100%; margin-top: 24px;">';

  // Social links section
  if (activePlatforms.length > 0) {
    html += `
      <div class="social-links-section" style="margin-bottom: ${customLinks.length > 0 ? '20px' : '0'};">
        <div 
          class="social-links-grid"
          style="
            display: flex;
            gap: 12px;
            justify-content: center;
          ...
          "
        >
          ${activePlatforms.map(({ platform, url }) => createSocialIcon(platform, url)).join('')}
        </div>
      </div>
    `;
  }
  ...
  if (customLinks.length > 0) {
    html += `
      <div 
        ...
        "
      >
        ${customLinks.map(link => createCustomLinkButton(link)).join('')}
      </div>
    `;
  }
  ...
  if (!document.querySelector('#interactive-links-styles')) {
